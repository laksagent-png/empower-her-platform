...
export async function createEvent(eventData: Omit<Event, "id">): Promise<string | null> {
  try {
    return await database.createDocument<Omit<Event, "id">>(EVENTS_COLLECTION, eventData);
  } catch (error) {
    if (import.meta.env.DEV) {
...
export async function updateEvent(id: string, eventData: Partial<Event>): Promise<boolean> {
  try {
    await database.updateDocument<Event>(EVENTS_COLLECTION, id, eventData);
    return true;
  } catch (error) {
    if (import.meta.env.DEV) {
...
export async function deleteEvent(id: string): Promise<boolean> {
  try {
    await database.deleteDocument(EVENTS_COLLECTION, id);
    return true;
  } catch (error) {
    if (import.meta.env.DEV) {
