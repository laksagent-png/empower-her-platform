...
import { CalendarIcon, Upload, X, Plus, Trash2, Image as ImageIcon } from "lucide-react";
import { z } from "zod";
...
const EventFormDialog = ({ open, onOpenChange, editingEvent, onSave }: Props) => {
  ...
  // Image upload (Mocked with generated images)
  const handleImageUpload = async () => {
    ...
    try {
      ...
      const url = `https://picsum.photos/seed/${randomId}/800/600`;
      updateField("coverImageUrl", url);
    } catch (error: any) {
      console.error("Upload error:", error);
      toast({ title: "Upload Failed", description: "Could not generate image.", variant: "destructive" });
    } finally {
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-5 pt-2">
          ...
          <Field label="Cover Image" required error={errors.coverImageUrl}>
            <div className="flex items-center gap-3">
              ...
              >
                ...
                {uploadingCover ? "Generating…" : "Generate Cover"}
              </Button>
              {form.coverImageUrl && (
                <div className="relative w-16 h-16 rounded-lg overflow-hidden border border-border">
                  <img src={form.coverImageUrl} alt="Cover" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                  <button
                    type="button"
          ...
          <Field label="Gallery Images">
            ...
            >
              <ImageIcon size={16} className="mr-1" />
              {uploadingGallery ? "Generating…" : "Generate Image"}
            </Button>
